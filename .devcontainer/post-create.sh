...
if ! command -v expect >/dev/null 2>&1; then
    ...
    command -v expect >/dev/null 2>&1 || {
        echo "Error: expect command not found after install attempt. Exiting."
        exit 1
    }
...
if ! command -v jq >/dev/null 2>&1; then
    ...
    command -v jq >/dev/null 2>&1 || {
        echo "Error: jq command not found after install attempt. Exiting."
        exit 1
    }
...
command -v expect >/dev/null 2>&1 || echo "Warning: 'expect' not found."
command -v jq >/dev/null 2>&1 || echo "Warning: 'jq' not found."
command -v htop >/dev/null 2>&1 || echo "Warning: 'htop' not found."
...
if [ -d "${WORKSPACE_FOLDER}" ]; then
    ...
    GIT_TEMPLATE_PATH="${WORKSPACE_FOLDER}/.devcontainer/.gitmessage"
    GIT_TEMPLATE_TARGET_PATH="${USER_HOME}/.gitmessage"
    echo "[post-create.sh] Configuring git commit template..."
    if [ -f "${GIT_TEMPLATE_PATH}" ]; then
        cp "${GIT_TEMPLATE_PATH}" "${GIT_TEMPLATE_TARGET_PATH}" || echo "Warning: Failed to copy commit template."
        git config --global commit.template "${GIT_TEMPLATE_TARGET_PATH}" || echo "Warning: Failed to set commit template config."
        echo "[post-create.sh] Set commit template."
    else echo "[post-create.sh] Warning: Commit template ${GIT_TEMPLATE_PATH} not found. Skipping config."; fi

    # 4. Add useful Git aliases
    echo "[post-create.sh] Adding Git aliases..."
    add_git_alias() {
        local alias_name="$1"
        local alias_command="$2"
        git config --global "alias.${alias_name}" "${alias_command}" || echo "Warning: Failed to set git alias '${alias_name}'."
    }
    add_git_alias "fblame" "blame -w -C -C -C"
    add_git_alias "st" "status"
    add_git_alias "co" "checkout"
    add_git_alias "cb" "checkout -b"
    add_git_alias "br" "branch"
    add_git_alias "logp" "log --pretty=format:'%C(yellow)%h %Cred%ad %Cblue%an %Cgreen%s' --date=short"
    add_git_alias "l" "log --oneline --decorate --graph"
    add_git_alias "pullr" "pull --rebase"
    add_git_alias "pushf" "push --force-with-lease"
    echo "[post-create.sh] Git aliases added."
...
if [ ! -d "${ZSH_CUSTOM}/plugins" ] || [ ! -O "${ZSH_CUSTOM}/plugins" ]; then
    sudo mkdir -p "${ZSH_CUSTOM}/plugins" || echo "Warning: Failed to create Zsh custom plugins directory."
    sudo chown -R "${TARGET_USER}:${TARGET_GROUP}" "${USER_HOME}/.oh-my-zsh" || echo "Warning: Failed to chown .oh-my-zsh"
fi
...
clone_zsh_plugin() {
    ...
    if [ ! -d "${target_dir}" ]; then
        echo "[post-create.sh] Cloning Zsh plugin $(basename ${target_dir})..."
        git clone --depth 1 "${repo_url}" "${target_dir}" || echo "Warning: Failed to clone Zsh plugin $(basename ${target_dir})."
    else echo "[post-create.sh] Zsh plugin $(basename ${target_dir}) already exists."; fi
}
clone_zsh_plugin "https://github.com/zsh-users/zsh-syntax-highlighting.git" "${ZSH_SYNTAX_HIGHLIGHTING_DIR}"
clone_zsh_plugin "https://github.com/zsh-users/zsh-autosuggestions.git" "${ZSH_AUTOSUGGESTIONS_DIR}"
ZSHRC_PATH="${USER_HOME}/.zshrc"
if [ -f "${ZSHRC_PATH}" ]; then
    echo "[post-create.sh] Modifying ${ZSHRC_PATH}..."
    sed -i '/^ZSH_THEME=/c\ZSH_THEME="agnoster"' "${ZSHRC_PATH}" || echo "Warning: Failed to set Zsh theme in ${ZSHRC_PATH}."
    if ! grep -q "plugins=(git zsh-syntax-highlighting zsh-autosuggestions)" "${ZSHRC_PATH}"; then
        echo "[post-create.sh] Enabling zsh-syntax-highlighting and zsh-autosuggestions plugins..."
        sed -i '/^plugins=/c\plugins=(git zsh-syntax-highlighting zsh-autosuggestions)' "${ZSHRC_PATH}" || echo "Warning: Failed to set Zsh plugins in ${ZSHRC_PATH}."
    fi
    HISTORY_DIR="/home/vscode/commandhistory"
    HISTORY_FILE="${HISTORY_DIR}/.zsh_history"
    echo "[post-create.sh] Configuring Zsh history file to ${HISTORY_FILE}"
    if [ ! -d "${HISTORY_DIR}" ]; then
        sudo mkdir -p "${HISTORY_DIR}" || echo "Warning: Failed to create history directory ${HISTORY_DIR}."
        sudo chown "${TARGET_USER}:${TARGET_GROUP}" "${HISTORY_DIR}" || echo "Warning: Failed to chown history directory ${HISTORY_DIR}."
    fi
    if [ ! -f "${HISTORY_FILE}" ]; then
        sudo touch "${HISTORY_FILE}" || echo "Warning: Failed to touch history file ${HISTORY_FILE}."
        sudo chown "${TARGET_USER}:${TARGET_GROUP}" "${HISTORY_FILE}" || echo "Warning: Failed to chown history file ${HISTORY_FILE}."
    fi
    ...
    if ! grep -q "HISTFILE=${HISTORY_FILE}" "${ZSHRC_PATH}"; then
        echo "[post-create.sh] Adding Zsh history settings..."
        printf "\n# Persistent History Config (Dev Container)\n" >>"${ZSHRC_PATH}"
        printf "HISTFILE=%s\n" "${HISTORY_FILE}" >>"${ZSHRC_PATH}"
        printf "HISTSIZE=10000\nSAVEHIST=10000\nsetopt APPEND_HISTORY\nsetopt SHARE_HISTORY\nsetopt INC_APPEND_HISTORY\n" >>"${ZSHRC_PATH}"
    fi
    # Add eza aliases only if eza was successfully installed (by feature)
    if command -v eza >/dev/null 2>&1; then
        echo "[post-create.sh] Adding eza aliases to ${ZSHRC_PATH}..."
        if ! grep -q "# Eza Aliases" "${ZSHRC_PATH}"; then
            printf "\n# Eza Aliases (Modern ls)\n" >>"${ZSHRC_PATH}"
            printf "alias ls='eza --color=auto --icons'\n" >>"${ZSHRC_PATH}"
            printf "alias l='eza --color=auto --icons'\n" >>"${ZSHRC_PATH}"
            printf "alias ll='eza -l --color=auto --icons'\n" >>"${ZSHRC_PATH}"
            printf "alias la='eza -a --color=auto --icons'\n" >>"${ZSHRC_PATH}"
            printf "alias lla='eza -la --color=auto --icons'\n" >>"${ZSHRC_PATH}"
        fi
    else echo "[post-create.sh] 'eza' not found, skipping eza aliases."; fi

    # *** REMOVED Keychain sourcing logic ***
    SOURCE_COMMENT="# Source SSH Agent variables from mounted keychain dir"
    # Check if the comment marker exists and remove it and the line after if found
    if sudo -u "${TARGET_USER}" grep -Fxq "${SOURCE_COMMENT}" "${ZSHRC_PATH}"; then
        echo "[post-create.sh] Removing keychain sourcing from ${ZSHRC_PATH}"
        # Use sed to delete the comment line and the line immediately following it
        sudo -u "${TARGET_USER}" sed -i -e "/^${SOURCE_COMMENT}\$/{N;d;}" "${ZSHRC_PATH}" || echo "Warning: Failed to remove keychain sourcing lines."
    else
        echo "[post-create.sh] Keychain sourcing not found in ${ZSHRC_PATH}."
    fi
...
if [ -f "${PYTHON_REQS_PATH}" ]; then if command -v pip3 &>/dev/null; then
    echo "[post-create.sh] Found ${PYTHON_REQS_PATH}, installing dependencies for user ${TARGET_USER}..."
    pip3 install --user --no-cache-dir -r "${PYTHON_REQS_PATH}" || echo "Warning: pip3 install failed."
else echo "[post-create.sh] Warning: pip3 command not found. Cannot install Python dependencies."; fi; else echo "[post-create.sh] No requirements file found at ${PYTHON_REQS_PATH}. Skipping pip install."; fi
